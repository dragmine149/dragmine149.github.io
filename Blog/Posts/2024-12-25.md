...
Your christmas gift from me, one special segements about how i do stuff. (I wanted to do two but one is enough and besides i need to sleep at some point)
...
but i believe overall i would only end up using a small amount of them and spending a lot just so i can have my own custom stuff. At that point might asa well make a website from scratch.
...
Still no. Besides this website isn't done yet, there are a couple more features i still wish to add which will be revealed in the upcomming months.

I wanted to make a blog first as a place where i can actually talk. (more on that later), and a blog is one of the easiest things to make. On top of that, doign the easy thing first means i can make some other systems and get use to them to help improve other
aspects of the site.
...
#### Hidden navigation
As i mentioned in a previous blog, i made the navigation bar after the main page hence i got used to the main pag look and wanted to keep it minimilistic.
...
Having a standard white background would have been a bit too booring for me, same goes for any solid colour. Hence a dynamically changing day night cycle. I wanted something a bit more smooth which i'm going to have to work on in the future.
...
What i'm complainging about is how websites use the middle third and the middle third only, that's 2/3rds of realestate unused. Even just using the middle 50% and leaving the outer 25% would be better.
...
The alternative to using markdown would be a plain txt file, but markdown gives more feautres. Or writting out every line one by one with all the code tags, yeah no thanks. That would take me forever as i attempt not to make the blog look bad due to the placement
of the code tags.
...
But Zed is a code editor, and due to this there is no spelling and grammer checking, nor is their any extensions. Even for files like a `.txt` or `.md`. I'm not complainging, tbh i think it's better without a spell checker built in.
...
Simple, i think of something and start writting about it until i can't think of anything else to write about it. And then i move on and repeat, until i believe the blog is done which is when i commit and publish said blog.

I don't really care if what i say is contreversal or it's something that could get me into trouble. I don't even really go back and check what i wrote even when doing the spelling rounds.
...
Also i want to express my own fellings which is already hard enough to do in text, so the more i write without caring about what i write the better.

### Why i write blogs
To talk, this is my current and only way of communicating... efficently.

TLDR, over this past year (i believe, fellings are hard) that i've been slightly depressed. (slightly, more like majourly. If it wasn't for anime then well yeah you kinda get the picture here and if you don't, don't worry please.)
...
So yeah, i'm going to continue writting blogs in my style for hopefully a long time. But at least for now to keep partialy sane (and so i have more than one place of backup).
...
## Anyway, i forgot to mention this eariler. But... MERRY CHRISTMAS
...
Outside of Advent of code, december has just been a bog standard month with every day being the same as the last, nothing special like christmas music or christmas games (as stated in an eariler blog this month).

But still if you celebrate christmas, i hope you are enjoying yourself and please do, someone has to enjoy christmas. You're situtation is different to mine, so at least enjoy it better than i do.

## Alright, out of time and ideas.
It's currently 1:30 AM on december 25th 2o24. I need to go sleep soon otherwise i'm not going to wake up early enough for our guests tomorrow. (well today). I'm also running out of things to put in this blog without going into some other category that is way out of
wack for this blog.
...
One thing for sure, i'll guarentee another blog before the end of the year, so look out for that.
