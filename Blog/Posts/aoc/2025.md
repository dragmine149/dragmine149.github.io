...
### Utility stuff
So i merged part of 2024 utilty program into this one, it works, but its like twice as slow. Which makes sense in a way as it's more general code and not specific code but twice as slow is a major drop. I've left the code hanging in a loose commit for
now until i work out what to do with it. It would be nice to use, but i do also want to make sure my code runs decently quick...

## Day 5
<a href="https://github.com/dragmine149/DragAOC/blob/main/aoc-2025/src/day05.rs">
  <img src="https://raw.githubusercontent.com/dragmine149/DragAOC/refs/heads/main/.aoc_tiles/tiles/2025/05.png" width="161px">
</a><br>

There is really not much to talk about today... It's a very easy day.

### Part 1
And back to day 2 we go, well kind of. It's another set of ranges, which means the typical
```rs
/// Where (usize, usize) => (start, end)
pub struct FreshRange(usize, usize);
```
And because valid ingredients are within the range, a simple double-ended check is all we need.
```rs
impl FreshRange {
...
```
Then we just loop, map and sum.
```rs
/// input is (Ranges, Ingredients)
input
    .1
    .iter()
    .map(|ingredient| {
    ...
    .sum()
```
*only released this after, but assigning to `usize` works as well and is much nicer to read*
```rs
...
### Part 2
Oh, we want all of them now? This fells like the community made Day 2 Part 3. Well then...

My first thought was just to make a massive vec containing every single number of every single range. and then unique-ify it.
```rs
/// new input generator, input is now only the ranges
input.iter().map(|r| r.0..r.0).flatten().unique().len();
```
Which took way too long to the point i canceled it after a couple of seconds and looked at the input file to see MASSIVE numbers. And as soon as you have massive numbers, you can't do slow ways anymore...

Okay, so lets just do `range_end - range_start` for each range. Nothing can go wrong right?
```rs
impl Freshrange {
...
```
```rs
input.iter().map(|r| r.range_count()).sum()
```
> That's not the right answer; your answer is too high.

Too high? What do you mean too high? *looks at example*. Oh. Ranges overlap. Alright, just need to merge them then. Simple enough. The answer was still too high after that. That is when i took a break for breakfast.

Coming back from breakfast and relooking at it. (especially the debug output of the ranges), multiple iterations of merging all the ranges where required as some merges would open new possibilites. SO shoving my code in a function, looping whilst the
new vector was not equal to the old vector gave me... the same answer.

I was already sorting the output to make it easier to read, but that sort is also what made it able to complete and get the right answer.

Very easy, didn't take long, hence can explain easier. *Maybe i need to rethink how i explain days*

## Day 6
<a href="https://github.com/dragmine149/DragAOC/blob/main/aoc-2025/src/day06.rs">
  <img src="https://raw.githubusercontent.com/dragmine149/DragAOC/refs/heads/main/.aoc_tiles/tiles/2025/06.png" width="161px">
</a><br>

*This input is going to come and bite my isn't it?*
Spoiler: it did.

Today is `advent of parsing` (as per what a friend called it) so we'll just get the solution out of the way first
```rs
impl Calculate {
...
```
Yep thats it, for both parts.

And now time for the parsing nightmare

### Part 1
Upon reading part 1, the first thing one might notice is
```
123 328  51 64 
 45 64  387 23 
  6 98  215 314
*   +   *   +  
```
The gaps in between the numbers `_ _ 6` for example. As you might have guessed, this does come back to bite us in part 2, but for now we can just ignore it. So, how do we parse the input? Well, we check the last line first.
```rs
...
// typical add the left over to the list one.
calculations.push(Calculation {
    numbers: vec![],
    operation: Operation::from(previous_char),
    size: size_count,
...
```
The last line contains all the operations, which just so happens to be at the start of the number. This means, we know if the rows above where each set of numbers "start". Technically i could have just split by whitespace, but as we need them later, i
did a bit of preperation in advance. Now that we have all the operations and the size of the operation, we can just find the numbers and add them to the list.
```rs
input
    .lines()
    .enumerate()
...
```
The key part here, is `line[pos..pos + size]` which gets us a sub-string of the line we are looking at, which is exactly the length of what we want. There is the catch of having to add
```rs
let size = if calc.size == 1 {
    line.len() - pos
} else {
...
```
Just because we keep offseting `pos` by the size and the left over ones we add have size `1` due to being at the end of the string.

### Part 2
Thankfully, not much had to be changed, instead we just add a bunch of new code.
```rs
calc.raw_numbers.push(line[pos..pos + size].to_string());
```
The only bit we needed to change, is instead of parsing them. Just add them to a list.

And then we can parse them.
```rs
calculations.iter_mut().for_each(|calc| {
    let length = calc
        .raw_numbers
...
```
This part took ~20 minutes, mainly in having to modifiy the original parser.
